  ...
  const onChangePermission = async (
    ...
    if (type === "agency") {
      await saveActivityLogsNotification({
        ...
        description: `Gave ${userData?.name} access to | ${
          subAccountPermissions?.Permission.find(
            (p) => p.subAccountId === subAccountId
          )?.SubAccount.name
        } `,
        subaccountId: subAccountPermissions?.Permission.find(
          (p) => p.subAccountId === subAccountId
        )?.SubAccount.id,
      });
    }
    ...
    if (response) {
      ...
      if (subAccountPermissions) {
        subAccountPermissions.Permission.find((perm) => {
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            />
            {/* This section is all about the user Role */}
            <FormField
              disabled={form.formState.isSubmitting}
              control={form.control}
              name="role"
            ...
            {/* This section mainly giveing the permission to access the subaccount to a perticular user */}
