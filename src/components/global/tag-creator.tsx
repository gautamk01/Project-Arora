"use client";
//when a user searches for a input we needed to filter out the tag
// if the user is search that and show them
//we need color and name for creating the tags
...
type Props = {
  subAccountId: string;
  getSelectedTags: (tags: Tag[]) => void; //array of tag
  defaultTags?: Tag[]; //array of tags
};

const TagColors = ["BLUE", "ORANGE", "ROSE", "PURPLE", "GREEN"] as const; //tag color type
export type TagColor = (typeof TagColors)[number];

const TagCreator = ({ getSelectedTags, subAccountId, defaultTags }: Props) => {
  const [selectedTags, setSelectedTags] = useState<Tag[]>(defaultTags || []); //selected tag
  const [tags, setTags] = useState<Tag[]>([]); // tag on the pages
  const router = useRouter();
  const [value, setValue] = useState("");
  const [selectedColor, setSelectedColor] = useState("");

  useEffect(() => {
    //every time the selected tag change we will give the selected tag to the function by props
